...
test-unit:
    mkdir -p results
    docker run --rm --volume $(shell pwd):/opt/calc --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest pytest --cov --cov-report=xml:/opt/calc/results/coverage.xml --cov-report=html:/opt/calc/results/coverage --junit-xml=/opt/calc/results/unit_result.xml -m unit || true
    docker run --rm --volume $(shell pwd):/opt/calc --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest junit2html /opt/calc/results/unit_result.xml /opt/calc/results/unit_result.html
